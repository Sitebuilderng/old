...
{else}
<form action="/actions/FormSend/" method="post" class="form {if $basketForm}formProduct addToBasketForm{/if}" enctype="multipart/form-data" >
{/if}
...
<label class="label_{$input.label|css_safe} {if $input.required=="yes"}required_label{/if} type_{$input.type}">{$input.label}</label>
{/if}
...
<p class="stockAndPrice">
	...
	<p id="success" class="Icon_Tick" style="display:{if $form_sent!="true"}none{else}block{/if}";>Thank you - this was successfully sent</p>
	{if $basketForm}
