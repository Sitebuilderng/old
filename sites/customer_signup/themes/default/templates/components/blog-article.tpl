...
<p class="blogDate clearfix">
	...
	{$timestamp|date_format_locale:"%A, %B %e, %Y":$language} <br/>
	...
	{if $in_categories|@count>1}
		...
		<br/>
	{/if}
	{if $tags}<span style='float:left;margin-right:10px'>{$langs.Tags}</span> 
	...
	<a href="#comments">{$langs.Comments}</a>: {$comments|@count} 
</p>
