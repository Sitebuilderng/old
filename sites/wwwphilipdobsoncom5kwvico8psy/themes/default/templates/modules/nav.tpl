<div class="module clearfix nav {if $logo}logo-{$logo}{/if} {if $search}search-{$search}{/if} {if $basket}basket-{$basket}{/if}  {if $align}align-{$align}{/if} {if !$width}{if $valign}valign-{$valign}{if $valign} valign{/if}{/if}{/if} {if $clear}clear-{$clear}{/if} {if $style}style-{$style}{/if} {if $width}width width-{$width} width-valign-{$valign}{/if}">
	<ul class="flexibreak-container contains-text clearfix">
		...
		<li class="current {if $item.subs}has-subs{/if} {if $smarty.foreach.loop1.iteration>$half}second-half{/if}"><a href="{if $item.homepage == "yes"}/{else}/{$item.url}/{/if}" title="{$item.title|replace:"Media Page":"Media"}">{if $item.subs}<span>{/if}{$item.title|replace:"Media Page":"Media"}{if $item.subs}</span>{/if}</a>{include file="nav/drop-down-menu.tpl" subs=$item.subs}</li>
		{assign var=current value="true"}
		{else}
		<li {if $item.subs}class="has-subs  {if $smarty.foreach.loop1.iteration>$half}second-half{/if}"{/if}><a href="{if $item.homepage == "yes"}/{else}/{$item.url}/{/if}" title="{$item.title|replace:"Media Page":"Media"}">{if $item.subs}<span>{/if}{$item.title|replace:"Media Page":"Media"}{if $item.subs}</span>{/if}</a>{include file="nav/drop-down-menu.tpl" subs=$item.subs}</li>
		{/if}
		{/if}
		{/foreach}
