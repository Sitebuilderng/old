...
{/if}
	...
	{if $input.type == "short"||$input.type == "phone"||$input.type == "name"}
		<input type="text" title="{$input.label}" name="{$input.id}" class="input {if $input.date}default_datepicker{/if} input_{$input.label|css_safe}{if $input.required=="yes"} required{/if}" value="{$input.value}{if $smarty.foreach.loop1.iteration==$inputpop}{$val}{/if}{$prepops.$x}"/>
	{/if}
	{if $input.type == "email"}
		<input type="text" title="{$input.label}" name="{$input.id}" class="input input_{$input.label|css_safe}{if $input.required=="yes"} required{/if}" id="emailInput" value="{if $smarty.foreach.loop1.iteration==$inputpop}{$val}{/if}{$input.value}{$prepops.$x}"/>
	...
	{/if}
	{if $input.type == "long"}
		<textarea rows="5" title="{$input.label}" cols="40" name="{$input.id}" class="textarea textarea_{$input.label|css_safe}{if $input.required=="yes"} required{/if}">{if $smarty.foreach.loop1.iteration==$inputpop}{$val}{/if}{$input.value}{$prepops.$x}</textarea>

	{/if}
	{if $input.type == "file"}
		<input type="file" title="{$input.label}" name="{$input.id}" value="" class="inputFile inputFile_{$input.label|css_safe}{if $input.required=="yes"} required{/if}"/>
	{/if}
